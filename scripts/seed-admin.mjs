...
// Initialize with project ID (uses Application Default Credentials from firebase login)
process.env.GCLOUD_PROJECT = "no2026";
process.env.GOOGLE_CLOUD_PROJECT = "no2026";

initializeApp({ projectId: "no2026" });
...
async function seed() {
  ...
  for (const c of clients) {
    const ref = await db.collection("clients").add(c);
    console.log(`  ✔ ${c.company} → ${ref.id}`);
  }

  console.log("\nA criar 10 leads...");
  for (const l of leads) {
    const ref = await db.collection("leads").add(l);
